...
**The bridge solver is exact, and that exactness is doing less work than it sounds like.** It plays out every remaining card of a deal and returns the number of tricks declarer's side can force against a defence playing perfectly — a real answer, not an estimate. But it is only exact because it is handed all four hands face up. There is a test in `games/bridge/tests/test_solver.py` that makes this concrete: declarer holds A-Q, an opponent holds the king, and the solver plays the queen because it can *see* where the king is. A real player at the table cannot, and the whole skill of the game is what to do without that. Closing that gap is Phase 3's job (PIMC: sample many full deals consistent with what has actually been observed, solve each one exactly, combine) and is not built yet. Until it is, "optimal" here means optimal in a game nobody is really playing.
