...
from games.court_piece import action_values as action_values_module
from games.court_piece.action_values import DEPTH_GATE, action_values
from games.court_piece.rules import CourtPiece, CourtPieceState, TrumpCall, new_deal
from games.court_piece.tests.test_rules import card

CALL = TrumpCall(trump=Suit.SPADES, caller=0)


def _endgame(tricks: int, rng: random.Random) -> CourtPieceState:
...
def test_a_real_fresh_deal_returns_a_value_for_every_legal_card() -> None:
    ...
    game = CourtPiece()
    state = new_deal(CALL, rng=random.Random(3))
    result = action_values(game, state)
