...
**"Unaware," precisely defined, and cheaper to build than first thought.** Not degraded memory of trick history (an earlier, wrong framing) — "unaware" means knowing how to win a trick but having no sense of the wider hand. That's exactly what `trick_odds.py`'s myopic fallback already is: correct, exact single-trick reasoning, zero lookahead. So the persona is just a switch — always route to the fallback, never attempt PIMC, regardless of how many tricks remain — not new architecture, a config on the existing depth gate.

**Bridge persona set, settled:** baseline (near-optimal, both domains), aggressive (bias toward higher-tier bids and committal card play), conservative (the mirror), selfish (biased toward individually flashy lines over the partnership's actual best result), baiter (biased toward the less-committal card within `equivalence.py`'s already-proven-tied groups — genuinely free, the alternatives are proven equal value), unaware (always the myopic fallback, per above). Bidding style and card-play style are one combined profile per seat for v1, not independent dials — simplest complete version first, split later only if there's a real reason to.

**Small random jitter around each persona's temperature/bias, not a fixed constant — genuinely new, not a repeat of an existing pattern.** Unlike poker (see below), no earlier version of this exists to lean on. Keep the jitter small and explicitly bounded, and test the *edges* of the range specifically, not just the center — this is exactly the same risk `AGGRESSION_BIAS` already ran into once (a value that looked reasonable in the middle but broke at the extreme), so the same discipline applies here from the start rather than being discovered the hard way again.

**Forward note for poker, not bridge, not now:** the same jitter idea belongs there too eventually, once it's proven out on bridge first — poker's five personas currently use one fixed number each, same as bridge's did before this session.
...
**Scoring (locked, replacing an earlier draft of this section that didn't match how this is actually played):** fractional, per deal, in units of a "court." Only the winning side ever scores anything — the losing side's score is always zero for that hand, never negative. What the win is worth stacks two independent multipliers:

- **How** it was won: a "piece" (7+ tricks, won any other way) is worth 0.1 of a court; a "kot" (all 13 tricks, or the first 7 won consecutively starting from trick 1) is worth a full court (1.0) — a real bonus for a genuine shutout-or-better, not just "reached the majority."
- **Who** won it: the trump-caller's own side winning is worth 1x (less impressive — they had the information advantage of choosing trump); the non-calling side winning it off them is worth 2x (a clean multiplier, not an uneven scale). This is still a real strategic input for the decision engine — whether calling trump is +EV depends partly on this payout asymmetry, not just hand strength.

Match length is a session-level choice, not fixed to a race-to-N: first-to-N-courts or open-ended free play, either is supported. Running trump has no caller to apply the who-won multiplier against — see below.
...
- **Stretch, PIMC realism:** `games/bridge/pimc.py`'s `sample_unseen()` already does the easy, load-bearing part right — played cards are removed from the pool, hand sizes come from what's actually left, and every decision resamples fresh rather than reusing an earlier deal. What it doesn't do yet: condition the sample on *inferred* information from play itself, chiefly a revealed void (a player failing to follow a suit proves they hold zero cards of it, which should zero out that suit in their share of the pool for every future sample this hand, not just get discovered again by chance). The bidding-weighted refinement one paragraph up in Phase 3 is the same idea applied to auction information instead of play information — both are a move from "uniform over everything not yet seen" to "uniform over everything not yet *ruled out*," and both should land as a distinct, labeled refinement after plain PIMC, not bundled in. Voids are the more tractable one to build first (a hard constraint from the rules, not a Bayesian weight to calibrate) and would make PIMC's sampled worlds meaningfully more realistic without touching the solver at all.

### Court Piece build plan
- **Fixed trump (game 3):** done. Declare-phase only — one player sees their first 5 cards and calls trump, caller role rotates each hand; kot/piece scoring with the caller/non-caller multiplier, on top of trick resolution reused from `engine/trick_taking/`.
- **Running trump (extension):** done, and it confirmed the prediction above — genuinely a small addition on the same state machine, not a fourth build. `TrumpCall.trump` is `Suit | None`; `CourtPiece.apply()` sets it the instant a player, unable to follow suit, plays a card, applying to that same trick's own resolution (the trump-setting card can win outright). Scores flat, no caller/non-caller multiplier. One real correctness subtlety this surfaced, not anticipated in the original plan: the double-dummy solver's transposition table has to key on trump too, not just `(seat, hands)` — two different move orders can converge on identical remaining hands while having set trump to different suits along the way, and the old bridge-derived key would have silently conflated them. Fixed and covered by a dedicated regression test (`games/court_piece/tests/test_solver.py`) that shares one search instance across both trump values to actually exercise the cache, not just check each answer with a fresh table. Worth remembering if `engine/trick_taking/`'s solver is ever genuinely factored out as shared infrastructure (§5's earlier note about that): this key subtlety would need to travel with it.
