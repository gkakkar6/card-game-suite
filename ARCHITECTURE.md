...
- **Phase 2:** double-dummy solver. Flagged explicitly above as the highest-risk module in the project. **Verify the test suite the way Phase 1's was verified, not just written and trusted:** deliberately introduce specific plausible bugs one at a time (e.g. mis-handling a suit, an off-by-one in whose turn it is, a wrong base case in the search), confirm the suite actually catches each one, then revert. A suite that passes on the first try is weak evidence on its own — tests written against the same misunderstanding as the code would pass too. This is now a standing practice for anything flagged as high-risk in this document, not a one-off done for bridge Phase 1.
